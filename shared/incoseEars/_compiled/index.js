 ...
 */
function findTermSpans(text, term) {
    const spans = [];
...
const TERM_RULES = [
    {
        ...
        message: (t) => `"${t}" is not measurable. State a quantified threshold, e.g. a maximum response time or a numeric limit.`,
    },
    {
        ruleId: 'incose.vague-term',
        severity: 'warn',
        terms: VAGUE_WARN_TERMS,
        message: (t) => `"${t}" is vague. Name the specific behaviour the requirement covers, with a measurable criterion.`,
    },
    {
...
export function validateRequirementText(text) {
    ...
    const patternCarriesCondition = earsPattern === 'event-driven' ||
