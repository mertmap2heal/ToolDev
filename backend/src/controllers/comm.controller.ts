...
export async function getMessages(req: Request, res: Response) {
  try {
    res.json({ success: true, data: await svc.listMessages(req.params.busId) })
  } catch (e) { res.status(500).json({ success: false, error: (e as Error).message }) }
}
...
export async function createMessage(req: Request, res: Response) {
  try {
    ...
    res.status(201).json({ success: true, data: await svc.createMessage(req.params.busId, { name, messageId, direction, description, metadata }) })
  } catch (e) { res.status(500).json({ success: false, error: (e as Error).message }) }
}

export async function updateMessage(req: Request, res: Response) {
  try {
    res.json({ success: true, data: await svc.updateMessage(req.params.id, req.params.busId, req.body) })
  } catch (e) {
    const msg = (e as Error).message
...
export async function getFields(req: Request, res: Response) {
  try {
    res.json({ success: true, data: await svc.listFields(req.params.messageId) })
  } catch (e) { res.status(500).json({ success: false, error: (e as Error).message }) }
}
...
export async function createField(req: Request, res: Response) {
  try {
    ...
    res.status(201).json({ success: true, data: await svc.createField(req.params.messageId, { fieldName, parameterId, description, dataType, order, config }) })
  } catch (e) { res.status(500).json({ success: false, error: (e as Error).message }) }
}

export async function updateField(req: Request, res: Response) {
  try {
    res.json({ success: true, data: await svc.updateField(req.params.id, req.body) })
  } catch (e) { res.status(500).json({ success: false, error: (e as Error).message }) }
}

export async function deleteField(req: Request, res: Response) {
  try {
    await svc.deleteField(req.params.id)
    res.json({ success: true })
  } catch (e) { res.status(500).json({ success: false, error: (e as Error).message }) }
}
...
export async function reorderFields(req: Request, res: Response) {
  try {
    ...
    await svc.reorderFields(req.params.messageId, orderedIds)
    res.json({ success: true })
  } catch (e) { res.status(500).json({ success: false, error: (e as Error).message }) }
}
