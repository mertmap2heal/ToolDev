    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">Verification Plans</h2>
          {progress && (
            <WorkflowStepper
              steps={progress.steps}
              currentStepId={progress.steps.find((s) => s.stage === 'verification')?.id}
            />
          )}
          <div className="mt-6 bg-white border border-gray-200 rounded-lg p-6">
            <p className="text-gray-600">Verification Plans form will be implemented here</p>
          </div>
        </div>
        <div>
          {projectId && (
            <AIGuidancePanel projectId={projectId} stage="verification" />
          )}
        </div>
      </div>
    </div>
