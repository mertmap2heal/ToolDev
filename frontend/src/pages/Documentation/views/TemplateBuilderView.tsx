...
import type { Template, TemplateSection, SectionType } from '../types'
import { TEMPLATE_SCOPES, DOC_TYPES } from '../mockData'

const SECTION_TYPES: { value: SectionType; label: string }[] = [
  { value: 'text', label: 'Text' },
  { value: 'table', label: 'Table placeholder' },
  { value: 'artifact_block', label: 'Artifact placeholder' },
  { value: 'generated_block', label: 'Generated placeholder' },
]
...
export default function TemplateBuilderView({ template, onSave, onBack }: TemplateBuilderViewProps) {
  const [name, setName] = useState(template?.name ?? '')
  const [docType, setDocType] = useState(template?.docType ?? 'SRS')
  const [scope, setScope] = useState(template?.scope ?? 'Project')
  const [sections, setSections] = useState<TemplateSection[]>(
    template?.sectionBlueprint ? [...template.sectionBlueprint] : []
  )
  ...
  const handleSave = () => {
    onSave({
      ...
      docType,
      scope,
      sectionBlueprint: sections.map((s, i) => ({ ...s, orderIndex: i })),
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
                ...
                <div className="flex items-center gap-2">
                  {editingId === sec.id ? (
                    <input
                      ...
                      className="flex-1 px-2 py-1 text-sm border rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                      autoFocus
                    />
                  ) : (
            ...
            {sections.length === 0 ? (
              ...
              sections.map((sec, i) => (
                <div key={sec.id} className="flex gap-2">
                  ...
                  <span>{sec.title}</span>
                  <span className="text-gray-400">({sec.type})</span>
                </div>
              ))
            )}
          </div>
        ...
        <div className="lg:col-span-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-4">
          ...
          <div>
            ...
            <select
              value={docType}
              onChange={(e) => setDocType(e.target.value as Template['docType'])}
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm"
            >
              {DOC_TYPES.map((opt) => (
                <option key={opt.value} value={opt.value}>
          ...
          <div>
            ...
            <select
              value={scope}
              onChange={(e) => setScope(e.target.value as Template['scope'])}
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm"
            >
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">Styling rules summary (placeholder)</p>
          <p className="text-xs text-gray-500 dark:text-gray-400">Export profile default (placeholder)</p>
          <button
            onClick={handleSave}
