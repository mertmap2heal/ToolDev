...
export default function CustomersTab() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (customerId: string) => inventoryService.deleteCustomer(customerId),
    onSuccess: () => {
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
          <p className="text-red-800 dark:text-red-200">Error loading customers</p>
        </div>
      ) : customers.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 dark:bg-gray-900 rounded-lg">
          <p className="text-gray-600 dark:text-gray-400 mb-4">No customers found</p>
          <button
            onClick={() => setIsCreateModalOpen(true)}
