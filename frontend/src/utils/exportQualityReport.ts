...
export function downloadQualityExcel(
  ...
  const summaryRows: (string | number)[][] = [
    ...
    ['Tool', 'Engineering Tool ΓÇö Requirement Quality Workbench'],
    [],
    ['Roll-up metrics', ''],
    ['Adjusted average score (0ΓÇô100)', stats.adjustedAverageScore],
  ...
  if (compare?.hasPrevious) {
    summaryRows.push(
      ...
      [
        ...
        `${compare.avgScoreDelta >= 0 ? '+' : ''}${compare.avgScoreDelta} (previous ${compare.avgScorePrevious ?? 'ΓÇö'}, current ${compare.avgScoreCurrent})`,
      ],
  ...
  // Column order: human-readable first; full UUID last with extra width (GUIDs are 36 chars ΓÇö narrow columns clip in Excel).
...
export function downloadQualityPdf(
  ...
  if (compare?.hasPrevious) {
    doc.text(
      `vs last run: avg ${compare.avgScoreDelta >= 0 ? '+' : ''}${compare.avgScoreDelta} (prev ${compare.avgScorePrevious ?? 'ΓÇö'}, now ${compare.avgScoreCurrent})`,
      40,
      86
    )
  }
