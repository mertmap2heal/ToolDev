...
read as the cached value never evaluated, 8 MB multer cap, 10 000-row cap). Two endpoints —
`POST /requirements/:projectId/import/xlsx/parse` (upload) + `.../commit` (commit) — tenant-scoped
by the existing router chain. The commit reuses the `bulkImportRequirements` validate-and-`$transaction`
substrate (extracted as `runBulkImport`) and returns a per-cell `{ rowNumber, column, field, severity,
...
**Status: Shipped — PR [#461](https://github.com/chriertcafdle-beep/ToolDevelopment/pull/461) / merge commit `4964772` (code commit `98f3c8a`) — Issue [#460](https://github.com/chriertcafdle-beep/ToolDevelopment/issues/460) (NX-7).** Delivered as one single-pipeline ticket: the shared `<ObjectiveCompletionMatrix>` component (`frontend/src/components/common/ObjectiveCompletionMatrix.tsx`), the backend objective-completion aggregation endpoint (`GET /certification/:projectId/objective-matrix`, `objectiveMatrix.service.ts` over the new extracted `certGraph.service.ts`), and the Requirements dashboard wired to render the matrix as its default body in place of the old count tiles. The cert-graph walk was extracted from the N-2.2 PSAC composer into a shared `certGraph.service.ts` (extract-then-reuse — both consumers walk one traversal). Two AC fields were reshaped to the schema reality with no migration: the `standard` filter is a server-side `regRef` classifier (`classifyRegRefStandard`) returning a bucket — `CertObjective` has no `standard` enum — and the matrix exposes `criticality` (Low/Medium/High), as `CertObjective` has no objective-level `applicableDal`. Wiring the matrix into the other four dashboards — Verification (objectives-with-test-coverage), Certification (objectives-satisfied), Validation (stakeholder-acceptance-per-objective), and the project landing as the default home view — remains deferred to named follow-on tickets (NX-7-followup-A..D). The 60-second `(projectId, baselineId)` cache below was deferred — the batched constant-query-count walk holds the SLA without it.
...
| CC6 | 11 audit tables → 1 universal provenance log | NX-10 (R-8 continuation) — the `SavedViewAuditEvent` reference migration manifests in *this* package: the table is written + read by `backend/src/controllers/traceabilityViews.controller.ts` (a Requirements / traceability-views surface). Status: **NX-10 shipped** — Issue [#471](https://github.com/chriertcafdle-beep/ToolDevelopment/issues/471) / PR [#472](https://github.com/chriertcafdle-beep/ToolDevelopment/pull/472) / merge commit `b24f2d9`. The `SavedViewAuditEvent` writer + reader are repointed onto the central `AuditLog`, history backfilled by `backfill-saved-view-audit-events.ts`, the legacy table kept frozen, and the reusable per-table audit-migration playbook codified in `.claude/kb/backend-patterns.md`. The other 9 outlier audit tables are named `NX-10-followon-*` tickets, one per sprint; `InventoryAuditLog` is dropped with the Inventory sunset, not migrated. See the closure entry in `improvements/_shared/cross-cutting.md`. |
