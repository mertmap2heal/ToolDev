...
| **RF-1 — Shared chrome + primitive library** — Issue #481 — In review (branch `feat/RF-1-chrome-primitives`) | Build the `_chrome.css` vocabulary as real React components/token classes: `Button`(+split/ghost/sm), `Tabs`, `StatusPill`, `DalChip`, `SeverityBadge`, `MonoChip`, `Avatar`, `StatTile`, `Banner`, `Card`, `FilterPill`, `Table` primitives. Restyle `Sidebar`/`Header` to the refresh look — **keep** the dynamic `MODULES` accordion, `Ctrl+B` collapse, `FeaturePackage` filtering, `GlobalSearch`. Re-enable `StatusBar` (D6). Replace the `Sparkles` AI icon with a neutral glyph (`design-system.md` §4). This IS the Phase-3 primitive library `ROADMAP-phase3.md` NX-6 called for. | frontend | M | RF-0 |
